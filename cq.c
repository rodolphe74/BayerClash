...
void
quantize (unsigned char *input_image, int width, int height, int comps, unsigned char *out_palette, int palette_size)
{
...
float		vv[MAXCOLOR], temp;
    ...
    for (int i = 0; i < size; i++) {
        Ir[i] = input_image[i * comps];
        Ig[i] = input_image[i * comps + 1];
        Ib[i] = input_image[i * comps + 2];
    }
    ...
    for (int i = 0; i < palette_size; i++) {
        out_palette[i * 4] = lut_r[i];
        out_palette[i * 4 + 1] = lut_g[i];
        out_palette[i * 4 + 2] = lut_b[i];
        out_palette[i * 4 + 3] = 255;
    }
